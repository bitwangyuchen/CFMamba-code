...
def inverse_ids_generate(origin_ids, K=1):
    '''
...
def mair_ids_generate(inp_shape, scan_len=4, K=1):
...
def mair_shift_ids_generate(inp_shape, scan_len=4, shift_len=0, K=1):
    inp_b, inp_c, inp_h, inp_w = inp_shape
    inp_idx = create_idx(1, 1, inp_h, inp_w)
...
def mair_ids_scan(inp, xs_scan_ids, bkdl=False, K=1):
    '''
        inp: B, C, H, W
        xs_scan_ids: K, 1, L
    '''
...
def test_time():
    ...
    # Original
    xs_scan_ids, xs_inverse_ids = mair_ids_generate(inp.shape, scan_len=scan_len, K=1)
